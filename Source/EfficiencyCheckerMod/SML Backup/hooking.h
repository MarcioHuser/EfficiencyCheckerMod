...
struct HookInvoker<R(C::*)(A...) const, PMF, TargetClass>
{
public:
    typedef CallScope<R(*)(const C*, A...)> ScopeType;
	// mod handler function
	typedef void HandlerSignature(ScopeType&, const C*, A...);
	typedef typename HandlerAfterFunc<R, const C*, A...>::Value HandlerSignatureAfter;
	typedef R HookType(const C*, A...);
	typedef R ReturnType;
...
public:
	...
	//When calling *static* functions returning by value, first argument is pointer to memory allocated by caller for storing return value.
	//But for calling *member* functions, first argument is always this pointer, so 
	static R* TrampolineFunctionCall(R* OutReturnValue, const C* self, A... args) {
		return (reinterpret_cast<R*(*)(const C*, R*, A...)>(functionPtr))(self, OutReturnValue, args...);
	}
	
    //Methods which return class/struct/union by value have out pointer inserted
    //as first parameter after this pointer, with all arguments shifted right by 1 for it
    static R* applyCallUserTypeByValue(const C* self, R* outReturnValue, A... args) {
        // Capture the pointer of the return value
        // so ScopeType does not have to know about that special case
        auto myTrampoline = [&](const C* self_, A... args_) -> R {
            (reinterpret_cast<R*(*)(const C*, R*, A...)>(functionPtr))(self_, outReturnValue, args_...);
            return *outReturnValue;
        };
	...
	//If it were returning user type by value, first argument would be R*, which is incorrect - that's why we need separate
	//applyCallUserType with correct argument order
	static R applyCallScalar(const C* self, A... args) {
    	ScopeType scope(handlersBefore, functionPtr);
    	scope(self, args...);
    	for (HandlerAfter& handler : *handlersAfter) handler(scope.getResult(), self, args...);
    	return scope.getResult();
	...
	//Call for void return type - nothing special to do with void
	static void applyCallVoid(const C* self, A... args) {
    	ScopeType scope(handlersBefore, functionPtr);
    	scope(self, args...);
    	for (HandlerAfter& handler : *handlersAfter) handler(self, args...);
    }
...
#if !WITH_EDITOR
		//TargetClass parameter matters when hooking virtual functions
		static_assert(std::is_base_of<const C, TargetClass>::value, "SUBSCRIBE_VIRTUAL_FUNCTION: TargetClass is not a child of function pointer class");
		FMemberFunctionStruct<T> MemberStruct{MemberFunctionPointer};
		FString ClassName = typeid(TargetClass).name();
		//Strip (class | struct | union ) from type name
		int32 FirstSpaceIndex;
...
template <typename R, typename C, typename... A, R(C::*PMF)(A...) const, typename TargetClass>
TArray<typename HookInvoker<R(C::*)(A...) const, PMF, TargetClass>::Handler>* HookInvoker<R(C::*)(A...) const, PMF, TargetClass>::handlersBefore = nullptr;

template <typename R, typename C, typename... A, R(C::*PMF)(A...), typename TargetClass>
TArray<typename HookInvoker<R(C::*)(A...), PMF, TargetClass>::HandlerAfter>* HookInvoker<R(C::*)(A...), PMF, TargetClass>::handlersAfter = nullptr;

template <typename R, typename C, typename... A, R(C::*PMF)(A...) const, typename TargetClass>
TArray<typename HookInvoker<R(C::*)(A...) const, PMF, TargetClass>::HandlerAfter>* HookInvoker<R(C::*)(A...) const, PMF, TargetClass>::handlersAfter = nullptr;

template <typename R, typename C, typename... A, R(C::*PMF)(A...), typename TargetClass>
R(* HookInvoker<R(C::*)(A...), PMF, TargetClass>::functionPtr)(C*,A...) = nullptr;

template <typename R, typename C, typename... A, R(C::*PMF)(A...) const, typename TargetClass>
R(* HookInvoker<R(C::*)(A...) const, PMF, TargetClass>::functionPtr)(const C*,A...) = nullptr;

template <typename R, typename C, typename... A, R(C::*PMF)(A...), typename TargetClass>
bool HookInvoker<R(C::*)(A...), PMF, TargetClass>::bHookInitialized = nullptr;

template <typename R, typename C, typename... A, R(C::*PMF)(A...) const, typename TargetClass>
bool HookInvoker<R(C::*)(A...) const, PMF, TargetClass>::bHookInitialized = nullptr;
